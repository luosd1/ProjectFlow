...
Agent Runtime 重构状态：T41 总方案、底座设计、Tools & Skills 设计、ADR 和研究文档已确认并提交（2026-07-04，commit `aecbb6c`）。Sidecar 骨架（S3）、Read-only Tools（S5）、AssignmentProposalTool（S8）、Skills 系统（S14）、Debug 模式（S16）已在 `agent-bridge/` 实现并通过验收（229 sidecar tests pass）。后续 Agent 底座工作必须先读 T41 文档，不要把旧 Coordinator 当成目标架构。
