...
- **AgentProposal**: Agent 高影响输出暂存（clarify/plan/breakdown），确认后才持久化到项目状态；reject 时记录 rejection_reason
...
| 24 | Agent Output Quality + Bug Fixes: 全中文输出、成员名/任务名替代 ID、max_tokens 适配推理模型、行动卡归属按 user_id、任务状态更新修复、evidence schema 修复、Bug 记录文档化（docs/T23/T23-C-feedback.md） | 已完成：2026-06-03 |
