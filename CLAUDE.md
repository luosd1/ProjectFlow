...
Agent Runtime 重构状态：T41 总方案、底座设计、Tools & Skills 设计、ADR 和研究文档已确认并提交（2026-07-04，commit `aecbb6c`）。Sidecar 骨架（S3）、Read-only Tools（S5）、Skills 系统（S14）、Debug 模式（S16）已在 `agent-bridge/` 实现并通过验收；S9 Check-in/replan migration 已完成（2026-07-05，commits `7e49836`、`800f578`），`generate_replan_proposal` internal tool 已注册，Agent 推断的 check-in task status changes 进入 pending replan proposal，不再直接调用 `create_status_update()`，已有 pending replan 时新调用返回 blocked。当前验收基线：302 backend tests pass，200 agent-bridge tests pass。后续 Agent 底座工作必须先读 T41 文档，不要把旧 Coordinator 当成目标架构。
...
├── agent-bridge/            # T41 TypeScript Agent Runtime Sidecar (S3/S5/S14/S16 + S9 tool registration 已完成)
│   ├── src/
