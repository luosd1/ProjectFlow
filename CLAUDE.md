...
| 27 | Code Review Hardening: 正则词边界替换防子串损坏, _validate_references 复用 common.py helpers, rejected-pairs 降级(修复 all-rejected 500), 协商创建 proposal status 校验, 中文约束关键词, docstring 对齐, 阶段名称替代 stage_id, assign 事件加入阶段过滤, 协商输出持久化尝试(Phase 29 已改为 timeline-only), _build_user_facing_assignment 分支合并 | 已完成：2026-06-03 |
| 28 | Frontend Redesign Migration: 三栏项目布局(sidebar+content+agent sidebar), 蓝金视觉体系(Instrument Serif+Inter字体), workspace 路由改为跳转项目, AppShell 全高布局, 新组件(project-layout/sidebar/content/agent-sidebar/new-project-dialog/workspace-content/compact-stat/direction-decision-view) | 已完成：2026-06-04 |
| 29 | Agent Output Quality & Reliability Hardening: 当前时间注入, 项目资源进入 Agent 上下文, 方向澄清增强字段, negotiate timeline-only, fallback 中文化, proposal/timeline 状态透明度, 回归测试 221 backend / 26 frontend | 已完成：2026-06-05 |
