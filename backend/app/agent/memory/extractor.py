...
def _get_replan_confirmed_stable_fields(
    output: ReplanOutput,
) -> dict:
...
def extract_replan_confirmed(
    ...
    *,
    ...
    # ── plan 记忆（始终创建）──
    impact_cn = output.impact
    reason_cn = output.reason
    plan_content = (
        f"项目「{project_name}」的计划已调整：{impact_cn}。"
    )
    plan_rationale = f"重排理由：{reason_cn}。来源：重排确认。"
    ...
    if _has_tradeoff_rationale(output):
        ...
        if all_reasons:
            ...
            if len(all_reasons) > 1:
                aggregated_cn = "；".join(sorted(all_reasons))
            else:
                aggregated_cn = all_reasons[0]

            tradeoff_content = (
                f"项目「{project_name}」的重排权衡：{aggregated_cn}。"
            )
    ...
    if _has_boundary_rationale(output):
        ...
        if boundary_changes:
            ...
            if len(sorted_boundaries) > 1:
                ...
                boundary_cn = sorted_boundaries[0]

            boundary_content = (
                f"项目「{project_name}」的范围调整：涉及「{boundary_cn}」等任务。"
            )
