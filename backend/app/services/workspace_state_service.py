...
def get_workspace_state(session: Session, workspace_id: str) -> WorkspaceStateResponse | None:
    ...
    for mem in membership_rows:
        ...
        members.append(MemberState(
            ...
            skills=json.loads(profile.skills) if profile and isinstance(profile.skills, str) else (profile.skills if profile else []),
    ...
    if project_row:
        ...
        project_state = ProjectState(
            ...
            status=project_row.status.value if hasattr(project_row.status, "value") else project_row.status,
