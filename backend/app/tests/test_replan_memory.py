...
def test_replan_reject_with_reason_creates_rejection(client: TestClient):
    """拒绝 repaln 且带有理由时创建 rejection 记忆。"""
    workspace, project, *_ = _create_full_fixture(client)
...
def test_changed_replan_source_supersedes_old(client: TestClient):
    ...
    with Session(get_memory_engine()) as session:
        ...
        # Write second plan memory with same source_id but different hash → supersede
...
def test_replans_confirm_path_not_modified_by_this_issue(client: TestClient):
    ...
    # Direct call to /replans/confirm — this path does NOT trigger memory extraction
...
# ─── AC3 + AC4: Tradeoff and boundary extraction via direct unit test ───────
...
def test_replan_tradeoff_memory_extracted(client: TestClient):
    ...
    workspace, project, stage, task, owner, *_ = _create_full_fixture(client)
    ...
    with Session(get_memory_engine()) as session:
        ...
        # Create an AgentEvent first (required FK for AgentProposal)
        ...
        # Create a proposal with stage adjustments (tradeoff rationale)
        ...
        assert "开发周期不足" in [c.content for c in candidates if c.memory_type == "tradeoff"][0]
...
def test_replan_boundary_memory_extracted(client: TestClient):
    ...
    with Session(get_memory_engine()) as session:
        ...
        # Create a proposal with task changes that have can_cut=true (boundary rationale)
        proposal = AgentProposal(
            id=str(uuid.uuid4()),
            project_id=project["id"],
            workspace_id=workspace["id"],
...
def test_replan_aggregation_multiple_same_type_adjustments(client: TestClient):
    ...
    with Session(get_memory_engine()) as session:
        ...
        # Create a proposal with multiple changes
        proposal = AgentProposal(
            id=str(uuid.uuid4()),
            project_id=project["id"],
            workspace_id=workspace["id"],
...
def test_replan_reject_idempotent(client: TestClient):
    ...
    # Reject again should be safe (proposal is already rejected, will raise error)
    # Instead, verify only 1 rejection from first call
    memories_resp = client.get(
        f"/api/projects/{project['id']}/memories",
        params={"viewer_user_id": project["created_by"]},
    )
...
# ─── Extractor deterministic (same pattern as direction/proposal tests) ────
...
# ─── Team visibility (same as direction/proposal confirm tests) ────────────
...
def test_replan_memory_team_visibility_all_members_can_see(client: TestClient):
    ...
    # Check all memories have team visibility
...
def test_replan_memory_extraction_failure_does_not_block_confirm(client: TestClient):
    ...
    # Confirm replan — business decision persisted even if memory extraction fails
    replan_resp = client.post("/api/agent/replan", json={"workspace_id": workspace["id"]})
    assert replan_resp.status_code == 200
    proposal_id = replan_resp.json()["proposal_id"]
...
# ─── plan confirm should NOT create memories ────────────────────────────────


def test_plan_confirm_still_no_memories_for_regular_plan(client: TestClient):
    """确认普通的 plan proposal（非 replan）不创建 ProjectMemory。"""
    # This verifies the change didn't accidentally trigger memories for all proposal types
    workspace, project, *_ = _create_full_fixture(client)
    ...
    assert memories_resp.status_code == 200
    memories = memories_resp.json()
    # Plan proposals don't trigger memory extraction
    plan_memories = [m for m in memories if m["memory_type"] == "plan"]
    assert len(plan_memories) == 0
