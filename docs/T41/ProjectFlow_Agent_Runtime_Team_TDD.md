...
DB 是事实源不代表 read path 可以修复事实。ProjectState、WorkspaceState、timeline slice、read-only Agent tools 都只能报告当前事实和派生视图，不能在读取时推进 Stage/Project 或修复 Task/owner/date。

当前代码里 `GET /api/projects/{project_id}/state -> get_project_state() -> _catch_up_stage_progress() -> try_advance_stage() -> session.flush()` 是需要移出的历史例外。这个 catch-up 会改 `Stage.status`、`Project.current_stage_id`、`Project.status`，属于 Primary Project State，只能留在明确 command/job 路径里。

明确规则：

- human-origin `TaskStatusUpdate` 可以在同一 command transaction 内确定性触发 `try_advance_stage()`；
- seed/import/direct DB edit 造成的 stale stage 由 `State Repair Command`、migration、admin maintenance 或 scheduled/internal job 修复；
- Agent 推断出的 task status、stage、owner、date、mitigation 变化必须进入 replan proposal；
- `get_project_state`、`get_workspace_state`、`get_timeline_slice` 这类 read-only view 不得调用会写入、`flush()` 或 `commit()` 的服务。
...
### 4.4 Tiered Write Boundary

ProjectFlow 不用“能不能写 DB”划分 Agent 权限，而按写入效果划分：
...
| Advisory Project Record | Risk、ActionCard 等不改 Project/Stage/Task 主事实的运营记录 | 可在 manifest 允许时幂等创建 |
| Primary Project State | Project direction/status/current stage、Stage、Task、finalized owner/status/date 等事实 | 不可直接写 |

`TaskStatusUpdate` 是边界风险：表面是历史记录，但当前服务会同步改 `Task.status`，并可能触发 Stage/Project 自动推进。Agent 推断出来的 task status change 必须走 replan proposal 或明确的人类来源 command，不能作为普通 advisory write。

Stage/Project 自动推进本身不是禁止项；禁止的是把推进藏在读取路径或 Agent 推断写入里。它只能发生在人类明确命令、proposal confirmation commit，或显式 State Repair Command 中。

现有 `agent_flow_service._persist_agent_output()` 不能被 sidecar 直接整体复用为 tool persistence，因为它包含三类不同效果：

- `AssignmentRecommendationOutput` 创建 `AssignmentProposal`，这是 typed proposal，可以保留，但不能 finalize owner。
- `RiskAnalysisOutput` / `ActivePushOutput` 创建 Risk/ActionCard，这是 advisory write，可以保留但要幂等并返回 created IDs。
- `CheckInAnalysisOutput.task_updates` 调 `create_status_update()`，会改 `Task.status` 并可能推进 Stage/Project；这条必须改为 replan proposal 或 human-origin command。

`RiskAnalysisOutput.requires_confirmation` 的含义也要重分层：high-severity Risk row 本身不需要 proposal confirmation；只有会改任务状态、owner、日期、阶段或项目状态的 mitigation 需要 confirmation。
...
- Pi `beforeToolCall` 对应 ProjectFlow policy gate。validation error、policy denied、aborted 都必须返回模型可见 observation，不能只写日志。
...
- ProjectFlow 需要自己的 serializable `AgentRunState`，带 `schema_version`。取消、恢复、失败都不能只靠内存中的 Pi session。
- OpenAI Agents SDK 的 human-in-the-loop/tool approval 机制保留为未来 `ToolExecutionApproval` 的参考，不进入当前目标架构。
- `tool_not_found`、validation failure、policy denial 应统一走 model-visible error formatter，而不是直接抛异常终止 run。
- 需要 `max_tool_concurrency` 和 manifest guardrails。即使 provider 支持 parallel tool calls，ProjectFlow harness 也必须按 manifest 限流。
...
- pending proposal、advisory links 和 paused run 必须记录 tool schema/version/model/provider/budget。恢复时如果 manifest version 不兼容，不能盲目 resume，必须要求重新生成或人工处理。
...
- ProjectFlow 当前不采用 LangGraph runtime，但必须采用显式状态机和 checkpoint 思维：每个 run 的当前位置、pending tool、已持久化副作用都要可查询。
...
- LangGraph pending writes 说明部分成功需要被记账。ProjectFlow 工具执行失败时，必须区分 `no_side_effect`、`event_persisted`、`proposal_persisted`、`advisory_record_persisted`、`commit_persisted`、`unknown`。
...
| 数据 | 事实源 | sidecar 是否可写 |
|---|---|---|
| Primary Project State: Project / Stage / Task / finalized owner/status/date | FastAPI DB | 否 |
| Advisory Project Record: Risk / ActionCard | FastAPI DB | 只能通过 manifest 允许的 FastAPI advisory endpoint |
| Reviewable Draft Record: AgentProposal / AssignmentProposal | FastAPI DB | 只能通过 AgentProposal 或 typed proposal endpoint 创建 |
...
- proposal/advisory tool 的成功定义是 FastAPI 已经持久化对应 draft/advisory record，且 tool result 返回 `proposal_id` 或 `created_ids`。
- `AssignmentProposal` 创建成功属于 `proposal_persisted`，但不是 `AgentProposal`；result 必须用 `created_ids` 或 typed proposal link 表达，不能让模型误以为 owner 已 finalized。
...
### 7.4 Proposal Confirmation / Tool Execution Approval 边界

- `proposal_confirmation` 是当前唯一的人类确认业务边界，由用户通过 FastAPI public API confirm、reject、commit。它不默认 resume 当前 model loop。
- 模型只能创建 `AgentProposal`、`AssignmentProposal` 或 advisory records，不能 confirm/reject/commit。
- `ToolExecutionApproval` 是未来扩展点，只用于 open-world、destructive、external connector 等执行前必须人工批准的工具；当前目标架构不实现 `waiting_for_tool_approval` 状态机。
- 当前 policy gate 只能 `allow`、`deny/block` 或返回 terminal observation，不能暂停等待人工批准。
...
| advisory_write | 允许创建 advisory record，不允许改主事实 | 创建 Risk、ActionCard |
...
- read-only tool 的无副作用定义包括：不能 `session.add()` / `session.delete()`，不能修改 ORM attribute，不能 `flush()` / `commit()`，不能调用可能执行这些动作的服务。
...
- advisory_write tool 必须 sequential，必须返回 created IDs，且不能写 Project/Stage/Task/owner/status。
...
- `task-status` 服务链必须重新分层：Agent 不能直接调用会修改 `Task.status` 的路径，除非请求来源是明确的人类提交。
- `RiskAnalysisOutput.requires_confirmation` 必须重新解释或改名为 mitigation confirmation，不能阻止 advisory Risk row 创建。
...
5. 实现 read-only tools：workspace state、conversation、proposal、timeline。
6. 重分层旧 Agent 副作用：check-in task updates 改 replan proposal，high Risk confirmation 改 mitigation confirmation，assignment 保持 typed proposal。
7. 实现 policy gate、event bridge、run state bridge、cancel/timeout。
8. 封装第一个 proposal tool，建议从 clarify 或 plan 开始。
9. 接入 frontend stream/timeline/proposal confirmation，但保持旧 path fallback。
10. 为每条迁移 flow 增加 parity tests、idempotency tests、side-effect reconciliation tests。
11. 迁移 breakdown、assign、risk、replan。
12. 移除或缩小 Coordinator legacy adapter。
...
- read-only tool 无副作用；
- public GET / internal read-only tool 不会推进 Stage/Project，也不会执行 stale-state repair；
...
- `recommend_assignment` 重试不重复创建相同 `AssignmentProposal`；
- high-severity Risk 创建本身是 advisory write，mitigation 主事实变化才创建 replan proposal；
- Agent check-in task update 不调用 `create_status_update()`，不直接改 `Task.status`；
...
- proposal confirmation 不默认 resume model loop；
