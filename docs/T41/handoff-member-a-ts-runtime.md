...
### Advisory Write Tools (create_risk / create_checkin / update_stage_progress)

**已实现（2026-07-05）。**

三个 advisory write 工具，允许 Agent 创建 advisory 记录：
...
| `update_stage_progress` | draft_only | true | proposal_persisted |
| `submit_tool_result` | N/A (human-only) | N/A | N/A |

- `create_risk`：创建 Risk 记录，直接写入，无需确认
- `create_checkin`：创建 CheckInCycle + CheckInResponse，直接写入
- `update_stage_progress`：创建 StagePlanProposal（proposal_type="plan"），需人工确认
- `submit_tool_result`：human-only 工具，前端调用以确认/拒绝提案，不暴露给 LLM

**验收标准：**
- [x] tool manifest 和 endpoint 实现完成
- [x] create_risk/create_checkin 直接写入 advisory 记录
- [x] update_stage_progress 创建 proposal 需确认
- [x] submit_tool_result human-only endpoint 实现
- [x] side_effect_status 正确区分
- [x] 幂等处理：同一 idempotency_key 返回缓存结果
- [x] retry.maxAttempts=1（advisory 工具不重试）
- [x] 333 sidecar tests + 54 backend tests 通过
...
- [ ] `../scripts/npm run test` / `lint` / `build` 通过（5 个预先存在的测试失败）
