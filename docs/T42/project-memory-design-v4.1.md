...
+ 本地检索引擎（sqlite-vec 向量 + SQLite FTS5 关键词 + RRF 融合 + jieba 中文分词）
+ 本地 embedding（bge-small-zh-v1.5，lazy 下载，FTS5 fallback）
+ Markdown 导出作白盒展示
```
...
→ MemoryIndexBackend.index()（sqlite-vec+FTS5）
...
4. 本地检索引擎为零配置默认（sqlite-vec+FTS5+RRF），`MemoryIndexBackend` 抽象保留未来可替换。
...
V1 默认部署零新服务、零外部 API、零模型进仓库：

```text
sqlite-vec（SQLite 向量扩展，进程内加载）
+ SQLite FTS5（SQLite 内置全文检索，jieba 中文分词，external content + Python 预分词）
+ sentence-transformers 本地 embedding（BAAI/bge-small-zh-v1.5，~95MB，lazy 下载）
+ RRF 融合
```

`MemoryIndexBackend` 是抽象层，V1 唯一实现是 `SqliteVecMemoryIndexBackend`。抽象保留是为了未来可替换（切 Mem0/Graphiti 不改业务层），但 V1 不实现其他 backend。

### Embedding 模型下载策略（向量默认 + lazy 下载 + FTS5 fallback）

bge-small-zh-v1.5 约 95MB，MIT license。**模型文件不进 git 仓库**（避免仓库永久膨胀 +95MB 和 git 历史污染）。策略：

1. **lazy-load**：模型不在进程启动时同步加载，首次检索时才加载到内存（加载耗时 ~2–3s，权重进内存后每句编码 10–30ms，CPU 即可，不需要 GPU）。
2. **首次下载**：首次启动检测模型不在本地缓存目录（`backend/data/models/bge-small-zh-v1.5/`，gitignore）→ 起一个 daemon thread（`threading.Thread(daemon=True)`，标准库一行，不是任务队列基础设施）后台下载 + 在日志/启动输出打印提示。下载源 HuggingFace，国内可设环境变量 `HF_ENDPOINT=https://hf-mirror.com` 加速。下载期间不阻塞启动、不阻塞检索。
3. **FTS5 fallback**：模型就绪前，向量检索不可用，检索立即退到 FTS5 关键词检索返回（不报错、不阻塞、不等下载）。模型就绪后自动切回向量+FTS5 混合。
4. **warmup 命令**：提供 `python -m app.memory.warmup` 预下载 + 预加载模型，演示前跑一次即零等待。README 写明。
5. **离线环境**：无网络时模型下不了，永久走 FTS5 fallback（召回质量降一档，不黑屏）。真离线部署可在另一台机器下好后把模型目录拷过去。

降级链（含模型未就绪）：

1. embedding 模型未就绪 → 跳过向量检索，只走 FTS5。
2. sqlite-vec 向量检索失败 → 退到 FTS5 关键词检索。
...
+ SqliteVecMemoryIndexBackend 作 V1 唯一实现（sqlite-vec + FTS5 + RRF + 本地 embedding）
+ Markdown 导出
```

### 为什么用 sqlite-vec

1. **零配置是 ProjectFlow 硬约束**。CLAUDE.md 明确"本地演示优先，零配置"。sqlite-vec 是 SQLite 扩展，进程内加载，演示启动仍是一条命令。Mem0+Qdrant 要起独立服务，破坏这个承诺。
...
3. **记忆量级小**。V1 只从 4 类正式事件写入，单项目估计几十到几百条记忆。sqlite-vec 召回质量对这个量级够用。
4. **可控**。sqlite-vec 自己写自己控，没有 Mem0 的 `infer=True` 改写内容、遥测默认开、自带 history DB 和治理表冲突等坑。
...
| 候选 | V1 | 理由 |
|---|---|---|
| sqlite-vec + FTS5 + RRF | **采用** | 零配置、本地、腾讯验证、量级够用 |
| Mem0 OSS + Qdrant | V1.1 候选 | 召回质量更高，但破坏零配置、坑多；V1 不实现，sqlite-vec 召回不够再评估 |
...
V3 假设这 4 类事件已存在，但核对源码发现没有一处直接对得上。必须先改造 service 层让正式事件可订阅：
...
| `proposal_rejected` | `reject_proposal()` 只翻 status + 存 `rejection_reason`，**不写 AgentEvent** | **不写 AgentEvent**。`AgentProposal` 本身就是拒绝的事实记录（`status=rejected` + `rejection_reason`）。extractor 在 `reject_proposal` 末尾直接调，`source_id=AgentProposal.id`。不扩 `AgentEventStatus`，不污染 timeline。 |
...
class ProjectMemorySync(SQLModel, table=True):
    ...
    backend: str = "sqlite_vec"       # V1 只有 sqlite_vec
...
2. extractor 内部开**新 session** 写 ProjectMemory（不复用正式事件的 session，避免回滚污染）。
3. 失败只记日志，**不抛异常、不回滚正式事件**（符合"记忆抽取失败不阻塞正式事件"）。
4. 延迟可接受：本地 sqlite-vec + FTS5 写入快，extractor + 索引同步约 50–200ms，对正式事件响应是可接受的一次性代价。
...
backend/app/agent/memory/sqlite_vec_backend.py # SqliteVecMemoryIndexBackend（V1 唯一实现）
...
→ MemoryIndexBackend.search()：
    - sqlite-vec 向量召回 top 50（模型就绪时；未就绪跳过）
    - FTS5 关键词召回 top 50（jieba 预分词）
    - RRF 融合两路 rank
...
3. **索引同步**：ProjectMemory 写入/删除/supersede 时同步 FTS5 行（这是 `MemoryIndexBackend.index()` 的职责，和 sqlite-vec 向量索引同步在一起做）。
...
1. embedding 模型未就绪 → 跳过向量，只走 FTS5。
2. sqlite-vec 向量失败 → FTS5 关键词检索（按 memory_type 优先级排序）。
...
1. **token 预算注入**：最多 1500 token（按 content + rationale 累加，超预算停），条数硬顶 8 条。不按固定条数限（单条长度不一，按条数限会信息过多或过少）。
...
8. AgentEvent 的 `output_snapshot` 加记忆使用 metadata：
   ```json
   {
     "memory_used": true,
...
```text
python -m app.memory.warmup    # 预下载 + 预加载 embedding 模型
```
...
如果 embedding 模型未就绪（首次下载中或离线），检索走 FTS5 fallback，`memory_backend = fts5`。
...
3. **执行**：mock LLM，调 `memory_service.get_context_for_agent`，断言：
   - 期望记忆召回率 `recall >= 90%`（允许 1 条漏召回，不硬卡 100%）
...
4. `source_type` 只允许 5 种枚举；`source_id` 统一指产生该决策的 proposal 对象 id。
...
9. 检索引擎不可用时降级到 FTS5 → 字段过滤 → none；模型未就绪时走 FTS5 fallback。AgentEvent 记录 `memory_backend` 和 `used_memory_ids`。
...
11. extractor 改 FastAPI BackgroundTasks（若同步延迟成问题）。
...
10. 检索引擎同步或检索失败不阻塞正式项目流程；走降级链（模型未就绪 → FTS5 → 字段过滤 → none）。
11. AgentEvent 记录 `memory_used`、`memory_backend`、`used_memory_ids`、`retrieval_latency_ms`；记忆使用不进 `AgentRunState.side_effects`。
...
16. **embedding 模型不进 git**：模型文件在 `backend/data/models/`（gitignore），首次启动 lazy 下载，就绪前检索走 FTS5 fallback，`python -m app.memory.warmup` 可预下载。
...
| P1-3 `memory_backend` 枚举 | 统一 `sqlite_vec \| fts5 \| field_filter \| none`（V1 无 mem0） | 「T41 集成边界」节埋点 + 降级链 |
| P1-4 eval recall 目标 | 统一 `>= 90%` | 「评估」节 + 验收标准 13 |
| P1-5 source_id 语义不一致 | P0-1 选 (b) 后统一 | 「source_id 语义统一」节 |
| P2-1 embedding 懒加载 | lazy-load 首次检索时加载 | 「Embedding 模型下载策略」节 |
| P2-2 `memory_service` 位置 | `services/memory_service.py` + `agent/memory/` | 「代码位置」节 |
| P2-3 100MB 模型下载 | 选方案 2：向量默认 + lazy 下载 + FTS5 fallback，不进 git | 「Embedding 模型下载策略」节 + 验收标准 16 |
...
| 用户决策 Q2 | 向量默认 + lazy 下载 + FTS5 fallback | 「Embedding 模型下载策略」节 |

## 与 V3 的差异

| 项 | V3 | V4.1 |
|---|---|---|
| 默认检索引擎 | Mem0 OSS + Qdrant | sqlite-vec + FTS5 + RRF（V1 唯一实现，Mem0 不进 V1） |
| 零配置 | 破坏（要起 Qdrant） | 保持 |
| 中文分词 | 未提 | jieba（FTS5 external content + Python 预分词） |
| embedding 模型 | 未提 | bge-small-zh-v1.5，lazy 下载，不进 git，FTS5 fallback |
| top_k | 12 | 50 |
| 注入限制 | 固定 6 条 | token 预算 1500，硬顶 8 条 |
| score 阈值 | 无 | cosine >= 0.3 可配 |
| 表结构 | ProjectMemory 单表 | ProjectMemory + ProjectMemorySync 拆表 |
| 幂等索引 | 未提 | 唯一索引 idx_memory_idemp + skip/supersede 三条规则 |
...
| 评测 | 无 | 最小 harness 进 CI + 量化指标（recall >= 90%） |
| reranker | V1 不做 | V1 不做（一致），明确 V1.1 加 |
| AgentEvent 埋点 | memory_used/backend/ids | 加 retrieval_count/injected_count/latency，memory_backend 枚举统一 |
...
本方案保持零配置默认部署（sqlite-vec 进程内，FTS5 内置，embedding 模型 lazy 下载不进 git，离线时 FTS5 fallback）。
