...
`memory_backend` 的取值 `sqlite_vec | fts5 | field_filter | none` 表示**本次请求实际生效的检索后端**，不是安装期承诺。默认安装通常解析为 `fts5`；只有向量 extra 已安装、扩展加载成功且模型就绪时，才可能解析为 `sqlite_vec`。
...
backend/app/agent/memory/sqlite_vec_backend.py # SqliteVecMemoryIndexBackend（optional memory-vector 增强）
...
→ MemoryIndexBackend.search()：
    - 默认 FTS5 关键词召回 top 50（jieba 预分词）
    - optional sqlite-vec 向量召回 top 50（memory-vector 安装且模型就绪时）
...
8. 回查后过滤 `status`、`visibility`、`valid_until`、`project_id`、score 阈值和当前 viewer 可见性。
9. Agent 按 token 预算（1500 token，硬顶 8 条）注入当前 viewer 可见的 ProjectMemory。
...
9. `superseded`、`archived`、过期、当前 viewer 不可见、项目不匹配、score 低于阈值的记忆不会注入 Agent 上下文。
...
12. 用户能在项目内查看当前 viewer 可见的只读结构化记忆，分类只含 5 个主题；能导出当前 viewer 可见的 Markdown；JSON 列表、Markdown 导出、Agent context 的可见记忆集合必须一致。
